...
p_norm <- dotplot(DEM_norm_KEGG,x="Count",showCategory = 5,includeAll=T,label_format = 50,
        color = "-log10(p.adjust)")+
            ...
            ggtitle("Young\nKEGG Enrichment of DEMs(OEGDF11 vs Control)")+
...
ggsave("/home/data5/dingyanheng/COP/pub_branch/GDF11/sh_rm/YoungKEGG_met.pdf",height = 6,width = 6)
#Senescence;
...
p_h2o2<-dotplot(DEM_H2O2_KEGG,x="Count",showCategory = 5,includeAll=T,label_format = 50,
        color = "-log10(p.adjust)")+
            theme_bw()+
            scale_fill_gradientn(colors=rev(met.brewer("Cassatt1")))+
            facet_grid(.~C1,shrink = F,drop = F)+
            ...
            theme(aspect.ratio = 1/1)+
            theme(plot.title = element_text(face = 'bold',size = 10))
ggsave("/home/data5/dingyanheng/COP/pub_branch/GDF11/sh_rm/SenKEGG_met.pdf",height = 6,width = 6)
