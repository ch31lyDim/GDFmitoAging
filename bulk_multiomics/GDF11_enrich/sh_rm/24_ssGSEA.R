...
library(dplyr)
library(MetBrewer)
expr = read.table("/home/data5/dingyanheng/COP/GDF11/Transcriptome/data/counts/Name_mat",sep = '\t',header = T)
rownames(expr) = expr$Name
expr = expr[,-1]
sample_info <- data.frame(sample = colnames(expr),
                          genotype = c(rep("C",6),rep("OE",6),rep("sh",6)),
                          condition = rep(c(rep("H2O2",3),rep("norm",3)),3),
...
gsea_res <- t(gsva(as.matrix(expr),list(AGING_MARKERS = aging_markers$gene_name,
                ...
                method="ssgsea",kcdf="Poisson",tau=0.5,abs.ranking=F,ssgsea.norm=T))
...
ggsave("/home/data5/dingyanheng/COP/pub_branch/GDF11/sh_rm/ssGSEA.pdf",width = 14,height = 7)
